...
ROI_MIN_BOUND = np.array([-1, -1, -1])
ROI_MAX_BOUND = np.array([1, 1, 1])
USE_BOUNDING_BOX_FILTER = False # As per user's provided code
...
def register_point_clouds_icp(source_pcd, target_pcd, initial_transform=np.identity(4),
                              ...
                              criteria=None): # Added criteria and max_dist params
    ...
    if not source_pcd.has_points() or not target_pcd.has_points():
        ...
        # Return the initial_transform and an empty point cloud or copy of source
    ...
    reg_p2p = o3d.pipelines.registration.registration_icp(
        ...
        max_correspondence_distance, # Use passed or global
        initial_transform,
    ...
    # Transform a fresh copy of the original source_pcd by the new transformation
...
if __name__ == "__main__":
    ...
    all_final_extrinsics = {} # To store refined extrinsics
    ...
    for i, cam_folder_name in enumerate(cam_folders_to_fuse):
        ...
        try:
            ...
            if not matching_depth_file.exists():
                matching_depth_file = depth_files[current_frame_idx] # Fallback
        ...
        pcd_to_process = o3d.geometry.PointCloud(current_pcd_in_world_full_res) # Start with full res transformed
        ...
        if USE_BOUNDING_BOX_FILTER and pcd_to_process.has_points():
            ...
            print(f"  Applied ROI bounding box filter. Points: {len(pcd_to_process.points)} (from {points_before_filter})")
            if not pcd_to_process.has_points():
                print(f"  Cloud empty after ROI filter for {cam_folder_name}. Skipping.")
                all_final_extrinsics[cam_folder_name] = np.copy(T_world_from_cam_extrinsic_initial) # Save initial if skipping
                continue
        
        pcd_for_icp_source = pcd_to_process # This is now filtered (if ROI) and in world coords
        ...
        if not pcd_for_icp_source.has_points():
            ...
            if pcd_to_process.has_points(): # Add the non-downsampled, possibly ROI-filtered cloud
        ...
        # --- ICP REGISTRATION ---
        if i == 0:
            master_fused_pcd = o3d.geometry.PointCloud(pcd_for_icp_source) # Base is the (filtered, downsampled) cloud
            print(f"  {cam_folder_name} is the first camera. Its cloud is the base.")
            processed_pcds_for_visualization.append(o3d.geometry.PointCloud(master_fused_pcd))
            all_final_extrinsics[cam_folder_name] = np.copy(T_world_from_cam_extrinsic_initial)
        else:
            if not master_fused_pcd.has_points():
                print("  Master fused PCD is empty, cannot register. Adding current cloud directly (if any) with initial extrinsic.")
                if pcd_for_icp_source.has_points(): # Add the (filtered, downsampled) cloud
                     master_fused_pcd = o3d.geometry.PointCloud(pcd_for_icp_source)
                     processed_pcds_for_visualization.append(o3d.geometry.PointCloud(master_fused_pcd))
                all_final_extrinsics[cam_folder_name] = np.copy(T_world_from_cam_extrinsic_initial)
                continue
            ...
            target_for_icp = master_fused_pcd
            if VOXEL_SIZE_DOWNSAMPLE_INPUT_PCD > 0: # Downsample target consistently with source
                 temp_target = master_fused_pcd.voxel_down_sample(VOXEL_SIZE_DOWNSAMPLE_INPUT_PCD)
                 if temp_target.has_points():
                     target_for_icp = temp_target
            
            # ICP is given source (pcd_for_icp_source) already in world coords (by initial extrinsic)
            # Initial guess for ICP is identity because source is already "close"
            icp_refinement_matrix, pcd_icp_registered_source = register_point_clouds_icp(
                pcd_for_icp_source, 
                target_for_icp,
                np.identity(4),
                ICP_MAX_CORRESPONDENCE_DISTANCE, # Pass configured max distance
                ICP_CONVERGENCE_CRITERIA       # Pass configured criteria
            )
            
            # The final extrinsic is the initial extrinsic followed by the ICP refinement
            ...
            # To add to master_fused_pcd:
            # Option 1 (current code's effective behavior): Add the (potentially downsampled) pcd_icp_registered_source
            # Option 2 (better for quality): Transform the full_res (but possibly ROI filtered) cloud with icp_refinement_matrix
            
            # Using Option 2 for adding to master for better quality if downsampling for ICP was done
            if pcd_for_icp_source is not pcd_to_process and VOXEL_SIZE_DOWNSAMPLE_INPUT_PCD > 0:
                # pcd_for_icp_source was a downsampled version of pcd_to_process.
                # Apply the found icp_refinement_matrix to the non-downsampled pcd_to_process
                pcd_to_add_to_master = o3d.geometry.PointCloud(pcd_to_process)
                pcd_to_add_to_master.transform(icp_refinement_matrix) # Apply refinement
                print(f"  Applied ICP refinement to non-downsampled (but possibly ROI filtered) cloud for merging.")
            else:
                # pcd_for_icp_source was not downsampled (or no downsampling was set),
                # or it's the same as pcd_to_process. So pcd_icp_registered_source is already the right one.
                pcd_to_add_to_master = pcd_icp_registered_source
    ...
    if final_pcd_to_show.has_points():
        ...
        if USE_BOUNDING_BOX_FILTER:
            ...
            bbox_lines.paint_uniform_color([0.8, 0.2, 0.2])
